\chapter{Third party infrastructure}
...
\noindent\sphinxincludegraphics[width=0.600\linewidth]{{startingpoint}.pdf}
...
\sphinxAtStartPar
Users on a common, shared computational infrastructure such as an {\hyperref[\detokenize{glossary:term-SSH-server}]{\sphinxtermref{\DUrole{xref,std,std-term}{SSH server}}}}
...
\noindent\sphinxincludegraphics{{clone_combined}.pdf}
\end{figure}
...
and/or third party storage providers (such as \sphinxhref{https://www.dropbox.com}{Dropbox}, \dlhbhref{G20}{Google},
...
illustrates the idea of using third party services as {\hyperref[\detokenize{glossary:term-special-remote}]{\sphinxtermref{\DUrole{xref,std,std-term}{special remote}}}}s from
which annexed file contents can be retrieved via \sphinxcode{\sphinxupquote{datalad get}}.

\sphinxAtStartPar
The upcoming section \DUrole{xref,std,std-ref}{gin} shows you one of the most easy ways to publish your
...
The section {\hyperref[\detokenize{basics/101-139-hostingservices:share-hostingservice}]{\sphinxcrossref{\DUrole{std,std-ref}{Publishing datasets to Git repository hosting}}}} (\autopageref*{\detokenize{basics/101-139-hostingservices:share-hostingservice}}) demonstrates how to publish datasets to any
kind of Git repository hosting service.
The sections \DUrole{xref,std,std-ref}{s3} and \DUrole{xref,std,std-ref}{dropbox} are concrete examples of sharing datasets
publicly or with selected others via different cloud services.
The section \DUrole{xref,std,std-ref}{gitlfs} talks about using the centralized, for\sphinxhyphen{}pay service
\dlhbhref{G13}{Git LFS} for sharing dataset content on GitHub, and the
section \DUrole{xref,std,std-ref}{figshare} shows built\sphinxhyphen{}in dataset export to services such as
\dlhbhref{F2}{figshare.com}.
...
%
\end{footnote}. This is the easiest way to share datasets and all their contents. Read on in the section \DUrole{xref,std,std-ref}{gin} or consult the tutorials of the \dlhbhref{D1G}{datalad\sphinxhyphen{}osf extension} to learn how to do this.
...
\item {} 
\sphinxAtStartPar
You can \sphinxstylestrong{export your dataset statically} as a snapshot to a service such as  \sphinxhref{https://figshare.com}{Figshare} or the \dlhbhref{O1}{OSF}\sphinxfootnotemark[1].
...
\caption{Schematic difference between the Git and git\sphinxhyphen{}annex aspect of your dataset, and where each part \sphinxstyleemphasis{usually} gets published to.}\label{\detokenize{basics/101-138-sharethirdparty:id8}}\end{figure}
...
The hosting services can be all kinds of private, institutional, or commercial services, and their location will be registered in the dataset under the concept of a {\hyperref[\detokenize{glossary:term-special-remote}]{\sphinxtermref{\DUrole{xref,std,std-term}{special remote}}}}.

\index{special remote@\spxentry{special remote}!git\sphinxhyphen{}annex concept@\spxentry{git\sphinxhyphen{}annex concept}}\index{git\sphinxhyphen{}annex concept@\spxentry{git\sphinxhyphen{}annex concept}!special remote@\spxentry{special remote}}\ignorespaces \begin{findoutmore}[label={index-0}, before title={\thetcbcounter\ }, check odd page=true]{What is a special remote}
...
to AWS S3, a \sphinxstyleemphasis{web} special remote downloads files from the web, and \sphinxstyleemphasis{datalad\sphinxhyphen{}archive}
...
\sphinxAtStartPar
Exemplary walk\sphinxhyphen{}throughs for \sphinxhref{https://www.dropbox.com}{Dropbox}, \sphinxhref{https://aws.amazon.com/s3}{Amazon S3 buckets}, and \dlhbhref{G13}{Git LFS} can be found in the upcoming sections in this chapter.
...
\noindent\sphinxincludegraphics[width=0.600\linewidth]{{clone_url}.pdf}
\end{figure}

\sphinxAtStartPar
If you are interested in learning how to set up different services as special remotes, you can take a look at the sections \DUrole{xref,std,std-ref}{s3}, \DUrole{xref,std,std-ref}{dropbox} or \DUrole{xref,std,std-ref}{gitlfs} for concrete examples with DataLad datasets, and the general section {\hyperref[\detokenize{basics/101-139-hostingservices:share-hostingservice}]{\sphinxcrossref{\DUrole{std,std-ref}{Publishing datasets to Git repository hosting}}}} (\autopageref*{\detokenize{basics/101-139-hostingservices:share-hostingservice}}) on setting up dataset siblings.
In addition, there are step\sphinxhyphen{}by\sphinxhyphen{}step walk\sphinxhyphen{}throughs in the documentation of git\sphinxhyphen{}annex for services such as \sphinxhref{https://git-annex.branchable.com/tips/public\_Amazon\_S3\_remote}{S3}, \dlhbhref{B1Q}{Google Cloud Storage},
\dlhbhref{B1R}{Box.com},
\dlhbhref{B1P}{Amazon Glacier},
\dlhbhref{B1O}{OwnCloud}, and many more.
Here is the complete list: \dlhbhref{B1M}{git\sphinxhyphen{}annex.branchable.com/special\_remotes}.
...
\sphinxAtStartPar
Read the section \DUrole{xref,std,std-ref}{gin} for a walk\sphinxhyphen{}through.
...
However, a number of {\hyperref[\detokenize{glossary:term-DataLad-extension}]{\sphinxtermref{\DUrole{xref,std,std-term}{datalad extension}}}}s have been created that equip cloud storage providers with the ability to also host Git repositories.
...
\noindent\sphinxincludegraphics[width=0.800\linewidth]{{publishing_network_publishosf}.pdf}
\end{figure}
...
\sphinxAtStartPar
DataLad provides the ability to do this out of the box to arbitrary locations, and support for specific services such as \dlhbhref{F2}{Figshare}.
Find out more information on this in the section \DUrole{xref,std,std-ref}{figshare}.
...
In the example below you can see that a public GitHub repository \sphinxurl{https://github.com/psychoinformatics-de/studyforrest-data-phase2} does not carry an annex on \sphinxcode{\sphinxupquote{github}} (the sibling \sphinxcode{\sphinxupquote{origin}}), but that the annexed data are served from an additional sibling \sphinxcode{\sphinxupquote{mddatasrc}} (a {\hyperref[\detokenize{glossary:term-special-remote}]{\sphinxtermref{\DUrole{xref,std,std-term}{special remote}}}} with annex support).
...
\PYG{g+go}{.: here(+) [git]}
\PYG{g+go}{.: mddatasrc(+) [http://psydata.ovgu.de/studyforrest/phase2/.git (git)]}
...
\noindent\sphinxincludegraphics[width=0.800\linewidth]{{GIN_newrepo}.png}
\caption{Webinterface of {\hyperref[\detokenize{glossary:term-GIN}]{\sphinxtermref{\DUrole{xref,std,std-term}{GIN}}}} during the creation of a new repository.}\label{\detokenize{basics/101-139-hostingservices:id6}}\end{figure}

\begin{figure}[tbp]
\centering
\capstart

\noindent\sphinxincludegraphics[width=0.800\linewidth]{{newrepo-github}.png}
\caption{Webinterface of {\hyperref[\detokenize{glossary:term-GitHub}]{\sphinxtermref{\DUrole{xref,std,std-term}{GitHub}}}} during the creation of a new repository.}\label{\detokenize{basics/101-139-hostingservices:id7}}\end{figure}
\begin{enumerate}
\sphinxsetlistlabels{\arabic}{enumi}{enumii}{}{.}%
\item {} 
\sphinxAtStartPar
...
\sphinxAtStartPar
Other streamlined arguments, such as \sphinxcode{\sphinxupquote{\sphinxhyphen{}\sphinxhyphen{}recursive}} or \sphinxcode{\sphinxupquote{\sphinxhyphen{}\sphinxhyphen{}publish\sphinxhyphen{}depends}} allow you to perform more complex configurations, such as publication of dataset hierarchies or connections to {\hyperref[\detokenize{glossary:term-special-remote}]{\sphinxtermref{\DUrole{xref,std,std-term}{special remote}}}}s. Upcoming walk\sphinxhyphen{}throughs will demonstrate them.
...
Thus, a short walk\sphinxhyphen{}through is at the {\hyperref[\detokenize{basics/101-139-hostingservices:gitlab}]{\sphinxcrossref{\DUrole{std,std-ref}{end of this section}}}} (\autopageref*{\detokenize{basics/101-139-hostingservices:gitlab}}).
...
\index{credential@\spxentry{credential}!storage@\spxentry{storage}}\ignorespaces \begin{findoutmore}[label={index-1}, before title={\thetcbcounter\ }, check odd page=true]{How does the authentication storage work?}
...
\noindent\sphinxincludegraphics[width=0.800\linewidth]{{github-token}.png}
...
\subsection{Creating a sibling on GitLab}
...
\sphinxAtStartPar
{\hyperref[\detokenize{glossary:term-GitLab}]{\sphinxtermref{\DUrole{xref,std,std-term}{GitLab}}}} is an open source Git repository hosting platform, and many institutions and companies deploy their own instance.
...
\sphinxAtStartPar
As a first step, users will need to create a configuration file following the format of \dlhbhref{R1A}{python\sphinxhyphen{}gitlab}.
This configuration file is typically called \sphinxcode{\sphinxupquote{.python\sphinxhyphen{}gitlab.cfg}} and placed into a users home directory.
...
\begin{sphinxVerbatim}[commandchars=\\\{\}]
\PYG{g+gp}{\PYGZdl{} }cat\PYG{+w}{ }\PYGZti{}/.python\PYGZhy{}gitlab.cfg
\PYG{g+go}{ [global]}
\PYG{g+go}{ default = my\PYGZhy{}university\PYGZhy{}gitlab}
\PYG{g+go}{ ssl\PYGZus{}verify = true}
\PYG{g+go}{ timeout = 5}

\PYG{g+go}{ [my\PYGZhy{}university\PYGZhy{}gitlab]}
\PYG{g+go}{ url = https://gitlab.my\PYGZhy{}university.com}
\PYG{g+go}{ private\PYGZus{}token = \PYGZlt{}here\PYGZhy{}is\PYGZhy{}your\PYGZhy{}token\PYGZgt{}}
\PYG{g+go}{ api\PYGZus{}version = 4}

\PYG{g+go}{ [gitlab\PYGZhy{}general]}
\PYG{g+go}{ url = https://gitlab.com}
\PYG{g+go}{ api\PYGZus{}version = 4}
\PYG{g+go}{ private\PYGZus{}token = \PYGZlt{}here\PYGZhy{}is\PYGZhy{}your\PYGZhy{}token\PYGZgt{}}
\end{sphinxVerbatim}

\sphinxAtStartPar
Once this configuration is in place, \sphinxcode{\sphinxupquote{create\sphinxhyphen{}sibling\sphinxhyphen{}gitlab}}’s \sphinxcode{\sphinxupquote{\sphinxhyphen{}\sphinxhyphen{}site}} parameter can be supplied with the name of the instance you want to use (e.g., \sphinxcode{\sphinxupquote{datalad create\sphinxhyphen{}sibling\sphinxhyphen{}gitlab \sphinxhyphen{}\sphinxhyphen{}site gitlab\sphinxhyphen{}general}}).
...
\noindent\sphinxincludegraphics[width=0.800\linewidth]{{gitlab-token}.png}
...
\sphinxAtStartPar
GitLab’s organization consists of \sphinxstyleemphasis{projects} and \sphinxstyleemphasis{groups}.
Projects are single repositories, and groups can be used to manage one or more projects at the same time.
In order to use \sphinxcode{\sphinxupquote{create\sphinxhyphen{}sibling\sphinxhyphen{}gitlab}}, a user \sphinxstylestrong{must} \sphinxhref{https://docs.gitlab.com/ee/user/group/\#create-a-group}{create a group} via the web interface, or specify a pre\sphinxhyphen{}existing group, because \dlhbhref{G10A}{GitLab does not allow root\sphinxhyphen{}level groups to be created via their API}.
...
\noindent\sphinxincludegraphics[width=0.800\linewidth]{{gitlab-rootgroup}.png}
...
\noindent\sphinxincludegraphics[width=0.800\linewidth]{{gitlab-rootgroup2}.png}
...
\begin{footnote}\sphinxAtStartFootnote
The default project name \sphinxcode{\sphinxupquote{project}} and path separator \sphinxcode{\sphinxupquote{\sphinxhyphen{}}} are configurable using the dataset\sphinxhyphen{}level configurations \sphinxcode{\sphinxupquote{datalad.gitlab\sphinxhyphen{}default\sphinxhyphen{}projectname}} and \sphinxcode{\sphinxupquote{datalad.gitlab\sphinxhyphen{}default\sphinxhyphen{}pathseparator}}
%
\end{footnote}.

\item {} 
\sphinxAtStartPar
\sphinxstylestrong{collection}:
...
\sphinxAtStartPar
Consider the \sphinxcode{\sphinxupquote{DataLad\sphinxhyphen{}101}} dataset, a superdataset with a several subdatasets in the following layout:

\begin{sphinxVerbatim}[commandchars=\\\{\}]
/home/me/dl\PYGZhy{}101/DataLad\PYGZhy{}101\PYG{+w}{    }\PYG{c+c1}{\PYGZsh{} dataset}
├──\PYG{+w}{ }books/
│\PYG{+w}{   }└──\PYG{+w}{ }\PYG{o}{[}...\PYG{o}{]}
├──\PYG{+w}{ }code/
│\PYG{+w}{   }└──\PYG{+w}{ }\PYG{o}{[}...\PYG{o}{]}
├──\PYG{+w}{ }midterm\PYGZus{}project/\PYG{+w}{    }\PYG{c+c1}{\PYGZsh{} subdataset}
│\PYG{+w}{   }├──\PYG{+w}{ }code/
│\PYG{+w}{       }└──\PYG{+w}{ }\PYG{o}{[}...\PYG{o}{]}
│\PYG{+w}{   }└──\PYG{+w}{  }input/\PYG{+w}{             }\PYG{c+c1}{\PYGZsh{} sub\PYGZhy{}subdataset}
├──\PYG{+w}{ }recordings/
│\PYG{+w}{   }└──\PYG{+w}{ }longnow/\PYG{+w}{    }\PYG{c+c1}{\PYGZsh{} subdataset}
│\PYG{+w}{       }├──\PYG{+w}{ }\PYG{o}{[}...\PYG{o}{]}
\end{sphinxVerbatim}

\sphinxAtStartPar
The \sphinxcode{\sphinxupquote{collection}} and \sphinxcode{\sphinxupquote{flat}} layouts for this dataset look like this in practice:

\begin{figure}[tbp]
\centering
\capstart

\noindent\sphinxincludegraphics[width=0.500\linewidth]{{gitlab-layouts}.png}
\caption{The \sphinxcode{\sphinxupquote{collection}} layout has a group (\sphinxcode{\sphinxupquote{DataLad\sphinxhyphen{}101\_collection}}, defined by the user with a configuration) with four projects underneath. The \sphinxcode{\sphinxupquote{project}} project contains the root\sphinxhyphen{}level dataset, and all contained subdatasets are named according to their location in the dataset. The \sphinxcode{\sphinxupquote{flat}} layout consists of projects in the root\sphinxhyphen{}level group. The project name for the superdataset (\sphinxcode{\sphinxupquote{DataLad\sphinxhyphen{}101\_flat}}) is defined by the user with a configuration, and the names of the subdatasets extend this project name based on their location in the dataset hierarchy.}\label{\detokenize{basics/101-139-hostingservices:id12}}\end{figure}


\subsubsection{Publishing a single dataset}
\label{\detokenize{basics/101-139-hostingservices:publishing-a-single-dataset}}
\sphinxAtStartPar
When publishing a single dataset, users can configure the project or group name as a command argument \sphinxcode{\sphinxupquote{\sphinxhyphen{}\sphinxhyphen{}project}}.
Here are two command examples and their outcomes.

\sphinxAtStartPar
For a \sphinxstylestrong{flat} layout, the \sphinxcode{\sphinxupquote{\sphinxhyphen{}\sphinxhyphen{}project}} parameter determines the project name:

\begin{sphinxVerbatim}[commandchars=\\\{\}]
\PYG{g+gp}{\PYGZdl{} }datalad\PYG{+w}{ }create\PYGZhy{}sibling\PYGZhy{}gitlab\PYG{+w}{ }\PYGZhy{}\PYGZhy{}site\PYG{+w}{ }gitlab\PYGZhy{}general\PYG{+w}{ }\PYGZhy{}\PYGZhy{}layout\PYG{+w}{ }flat\PYG{+w}{ }\PYGZhy{}\PYGZhy{}project\PYG{+w}{ }my\PYGZhy{}datalad\PYGZhy{}root\PYGZhy{}level\PYGZhy{}group/this\PYGZhy{}will\PYGZhy{}be\PYGZhy{}the\PYGZhy{}project\PYGZhy{}name
\PYG{g+go}{create\PYGZus{}sibling\PYGZus{}gitlab(ok): . (dataset) [sibling repository \PYGZsq{}gitlab\PYGZsq{} created at https://gitlab.com/my\PYGZhy{}datalad\PYGZhy{}root\PYGZhy{}level\PYGZhy{}group/this\PYGZhy{}will\PYGZhy{}be\PYGZhy{}the\PYGZhy{}project\PYGZhy{}name]}
\PYG{g+go}{configure\PYGZhy{}sibling(ok): . (sibling)}
\PYG{g+go}{action summary:}
\PYG{g+go}{  configure\PYGZhy{}sibling (ok: 1)}
\PYG{g+go}{  create\PYGZus{}sibling\PYGZus{}gitlab (ok: 1)}
\end{sphinxVerbatim}
...
\noindent\sphinxincludegraphics[width=0.500\linewidth]{{gitlab-layout-flat}.png}
\caption{An example dataset using GitLab’s “flat” layout.}\label{\detokenize{basics/101-139-hostingservices:id13}}\end{figure}

\sphinxAtStartPar
For a \sphinxstylestrong{collection} layout, the \sphinxcode{\sphinxupquote{\sphinxhyphen{}\sphinxhyphen{}project}} parameter determines the group name:

\begin{sphinxVerbatim}[commandchars=\\\{\}]
\PYG{g+gp}{\PYGZdl{} }datalad\PYG{+w}{ }create\PYGZhy{}sibling\PYGZhy{}gitlab\PYG{+w}{ }\PYGZhy{}\PYGZhy{}site\PYG{+w}{ }gitlab\PYGZhy{}general\PYG{+w}{ }\PYGZhy{}\PYGZhy{}layout\PYG{+w}{ }collection\PYG{+w}{ }\PYGZhy{}\PYGZhy{}project\PYG{+w}{ }my\PYGZhy{}datalad\PYGZhy{}root\PYGZhy{}level\PYGZhy{}group/this\PYGZhy{}will\PYGZhy{}be\PYGZhy{}the\PYGZhy{}group\PYGZhy{}name
\PYG{g+go}{ create\PYGZus{}sibling\PYGZus{}gitlab(ok): . (dataset) [sibling repository \PYGZsq{}gitlab\PYGZsq{} created at https://gitlab.com/my\PYGZhy{}datalad\PYGZhy{}root\PYGZhy{}level\PYGZhy{}group/this\PYGZhy{}will\PYGZhy{}be\PYGZhy{}the\PYGZhy{}group\PYGZhy{}name/project]}
\PYG{g+go}{ configure\PYGZhy{}sibling(ok): . (sibling)}
\PYG{g+go}{ action summary:}
\PYG{g+go}{   configure\PYGZhy{}sibling (ok: 1)}
\PYG{g+go}{   create\PYGZus{}sibling\PYGZus{}gitlab (ok: 1)}
...
\noindent\sphinxincludegraphics[width=0.500\linewidth]{{gitlab-layout-collection}.png}
...
\sphinxAtStartPar
When publishing a series of datasets recursively, the \sphinxcode{\sphinxupquote{\sphinxhyphen{}\sphinxhyphen{}project}} argument cannot be used anymore \sphinxhyphen{} otherwise, all datasets in the hierarchy would attempt to create the same group or project over and over again.
...
\index{configuration item@\spxentry{configuration item}!datalad.gitlab\sphinxhyphen{}\textless{}name\textgreater{}\sphinxhyphen{}project@\spxentry{datalad.gitlab\sphinxhyphen{}\textless{}name\textgreater{}\sphinxhyphen{}project}}\ignorespaces 
\def\sphinxLiteralBlockLabel{\label{\detokenize{basics/101-139-hostingservices:index-2}}}
\begin{sphinxVerbatim}[commandchars=\\\{\}]
\PYG{g+gp}{\PYGZdl{} }\PYG{c+c1}{\PYGZsh{} do the configuration for the top\PYGZhy{}most dataset}
\PYG{g+gp}{\PYGZdl{} }\PYG{c+c1}{\PYGZsh{} either configure with Git}
\PYG{g+gp}{\PYGZdl{} }git\PYG{+w}{ }config\PYG{+w}{ }\PYGZhy{}\PYGZhy{}local\PYG{+w}{ }\PYGZhy{}\PYGZhy{}replace\PYGZhy{}all\PYG{+w}{ }\PYG{l+s+se}{\PYGZbs{}}
\PYG{+w}{  }datalad.gitlab\PYGZhy{}\PYGZlt{}gitlab\PYGZhy{}site\PYGZgt{}\PYGZhy{}project\PYG{+w}{ }\PYG{l+s+se}{\PYGZbs{}}
\PYG{+w}{  }\PYG{l+s+s1}{\PYGZsq{}my\PYGZhy{}datalad\PYGZhy{}root\PYGZhy{}level\PYGZhy{}group/DataLad\PYGZhy{}101\PYGZus{}flat\PYGZsq{}}
\PYG{g+gp}{\PYGZdl{} }\PYG{c+c1}{\PYGZsh{} or configure with DataLad}
\PYG{g+gp}{\PYGZdl{} }datalad\PYG{+w}{ }configuration\PYG{+w}{ }\PYG{n+nb}{set}\PYG{+w}{ }\PYG{l+s+se}{\PYGZbs{}}
\PYG{+w}{  }datalad.gitlab\PYGZhy{}\PYGZlt{}gitlab\PYGZhy{}site\PYGZgt{}\PYGZhy{}project\PYG{o}{=}\PYG{l+s+s1}{\PYGZsq{}my\PYGZhy{}datalad\PYGZhy{}root\PYGZhy{}level\PYGZhy{}group/DataLad\PYGZhy{}101\PYGZus{}flat\PYGZsq{}}
\end{sphinxVerbatim}

\sphinxAtStartPar
Afterwards, publish dataset hierarchies with the \sphinxcode{\sphinxupquote{\sphinxhyphen{}\sphinxhyphen{}recursive}} flag:

\begin{sphinxVerbatim}[commandchars=\\\{\}]
\PYG{g+gp}{\PYGZdl{} }datalad\PYG{+w}{ }create\PYGZhy{}sibling\PYGZhy{}gitlab\PYG{+w}{ }\PYGZhy{}\PYGZhy{}site\PYG{+w}{ }gitlab\PYGZhy{}general\PYG{+w}{ }\PYGZhy{}\PYGZhy{}recursive\PYG{+w}{ }\PYGZhy{}\PYGZhy{}layout\PYG{+w}{ }flat
\PYG{g+go}{create\PYGZus{}sibling\PYGZus{}gitlab(ok): . (dataset) [sibling repository \PYGZsq{}gitlab\PYGZsq{} created at https://gitlab.com/my\PYGZhy{}datalad\PYGZhy{}root\PYGZhy{}level\PYGZhy{}group/DataLad\PYGZhy{}101\PYGZus{}flat]}
\PYG{g+go}{configure\PYGZhy{}sibling(ok): . (sibling)}
\PYG{g+go}{create\PYGZus{}sibling\PYGZus{}gitlab(ok): midterm\PYGZus{}project (dataset) [sibling repository \PYGZsq{}gitlab\PYGZsq{} created at https://gitlab.com/my\PYGZhy{}datalad\PYGZhy{}root\PYGZhy{}level\PYGZhy{}group/DataLad\PYGZhy{}101\PYGZus{}flat\PYGZhy{}midterm\PYGZus{}project]}
\PYG{g+go}{configure\PYGZhy{}sibling(ok): . (sibling)}
\PYG{g+go}{create\PYGZus{}sibling\PYGZus{}gitlab(ok): midterm\PYGZus{}project/input (dataset) [sibling repository \PYGZsq{}gitlab\PYGZsq{} created at https://gitlab.com/my\PYGZhy{}datalad\PYGZhy{}root\PYGZhy{}level\PYGZhy{}group/DataLad\PYGZhy{}101\PYGZus{}flat\PYGZhy{}midterm\PYGZus{}project\PYGZhy{}input]}
\PYG{g+go}{configure\PYGZhy{}sibling(ok): . (sibling)}
\PYG{g+go}{create\PYGZus{}sibling\PYGZus{}gitlab(ok): recordings/longnow (dataset) [sibling repository \PYGZsq{}gitlab\PYGZsq{} created at https://gitlab.com/my\PYGZhy{}datalad\PYGZhy{}root\PYGZhy{}level\PYGZhy{}group/DataLad\PYGZhy{}101\PYGZus{}flat\PYGZhy{}recordings\PYGZhy{}longnow]}
\PYG{g+go}{configure\PYGZhy{}sibling(ok): . (sibling)}
\PYG{g+go}{action summary:}
\PYG{g+go}{  configure\PYGZhy{}sibling (ok: 4)}
\PYG{g+go}{  create\PYGZus{}sibling\PYGZus{}gitlab (ok: 4)}
\end{sphinxVerbatim}


\subsubsection{Final step: Pushing to GitLab}
\label{\detokenize{basics/101-139-hostingservices:final-step-pushing-to-gitlab}}
\sphinxAtStartPar
Once you have set up your dataset sibling(s), you can push individual datasets with \sphinxcode{\sphinxupquote{datalad push \sphinxhyphen{}\sphinxhyphen{}to gitlab}} or push recursively across a hierarchy by adding the \sphinxcode{\sphinxupquote{\sphinxhyphen{}\sphinxhyphen{}recursive}} flag to the push command.

\index{configuration item@\spxentry{configuration item}!datalad.gitlab\sphinxhyphen{}default\sphinxhyphen{}projectname@\spxentry{datalad.gitlab\sphinxhyphen{}default\sphinxhyphen{}projectname}}\index{configuration item@\spxentry{configuration item}!datalad.gitlab\sphinxhyphen{}default\sphinxhyphen{}pathseparator@\spxentry{datalad.gitlab\sphinxhyphen{}default\sphinxhyphen{}pathseparator}}\ignorespaces 
\sphinxstepscope
...
\index{remove sensitive information@\spxentry{remove sensitive information}!with Git@\spxentry{with Git}}\index{with Git@\spxentry{with Git}!remove sensitive information@\spxentry{remove sensitive information}}\ignorespaces \begin{findoutmore}[label={index-0}, before title={\thetcbcounter\ }, check odd page=true]{Help! I accidentally saved sensitive information to Git!}
...
the {\hyperref[\detokenize{basics/101-141-push:fom-push-branch}]{\sphinxcrossref{\DUrole{std,std-ref}{find\sphinxhyphen{}out\sphinxhyphen{}more about this}}}} (\autopageref*{\detokenize{basics/101-141-push:fom-push-branch}}).
...
For more information on this, and other error messages during push, please checkout the section {\hyperref[\detokenize{basics/101-135-help:help}]{\sphinxcrossref{\DUrole{std,std-ref}{When things go wrong}}}} (\autopageref*{\detokenize{basics/101-135-help:help}}).
...
appropriate {\hyperref[\detokenize{glossary:term-special-remote}]{\sphinxtermref{\DUrole{xref,std,std-term}{special remote}}}}, and configure a publication dependency. The
section {\hyperref[\detokenize{basics/101-138-sharethirdparty:sharethirdparty}]{\sphinxcrossref{\DUrole{std,std-ref}{Beyond shared infrastructure}}}} (\autopageref*{\detokenize{basics/101-138-sharethirdparty:sharethirdparty}}) walked you through how this can be done with
\dlhbhref{D10}{Dropbox}.
